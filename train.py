...
from ordinalclip.runner.adience_data import RegressionDataModule
...
def load_callbacks(output_dir):
    ...
    return [
        ModelCheckpoint(
            monitor="val_acc_max_metric",
            dirpath=str(output_dir / "ckpts"),
            filename="{epoch:02d}-{val_acc_max_metric:.4f}",
            verbose=True,
            save_last=True,
            save_top_k=1,
            mode="max",
...
def main(cfg: DictConfig):
    ...
    runner = None
    regression_datamodule = RegressionDataModule(logger, **OmegaConf.to_container(cfg.data_cfg))
    ...
    if cfg.test_dir == 'No test dir':
        ckpt_paths = list((output_dir / "ckpts").glob("*.ckpt"))
    else:
        ckpt_paths = [cfg.test_dir]
...
def parse_cfg(args, instantialize_output_dir=True):
    config_files = args.config or ["configs/adience.yaml"]
    cfg = OmegaConf.merge(*[OmegaConf.load(config_) for config_ in config_files])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_dir", type=str, default='No test dir')
